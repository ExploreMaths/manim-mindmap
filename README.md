...
作为第一款思维导图插件(截至项目提交,没有找到同类 Manim 插件)，开发经历了大概三个阶段。截止到目前为止,又追加了时序图、组织目录结构图的实现。
...
导入的类如下：
+ Node：最基本的节点类，构成思维导图等结构
+ MindMap：一个思维导图类，支持四个布局方向
    + StandardMap：标准两侧布局(左右，或上下)
+ CatalogMap：一个目录、组织结构类
+ TimeLine：一个时序图类
+ 动画基类：LayoutAnimation(也可直接用)
...
在 LayoutAnimation 和 InsertNode 等动画类中，可以通过参数 layout_type 指定布局类型，默认为 LayoutType.MindMap。LayoutType 中定义了多种布局类型：
+ LayoutType.MindMap：思维导图布局（默认）
+ LayoutType.Standard：左右或上下两侧布局的思维导图
+ LayoutType.TimeLine：时间轴布局
+ LayoutType.Catalog：组织目录结构图
...
目录结构图的实现：CatalogMap 类与 MindMap 类用法相同。但是只有向下布局，不需要指定方向参数。动画效果如下：

<div align="center">
  <video src="https://github.com/user-attachments/assets/93143fe5-ee7d-4e5d-91ff-078b59943972" width="600" controls></video>
</div>